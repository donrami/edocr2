...
def recognize_gdt(img, block, recognizer):
    ...
    for i in range(1, len(block)):
        ...
        if new_line:
            ...
            pred += '|' + p
    # FIX: Remove digit filter to detect all FCFs including those without numbers
    # Original filter was: if any(char.isdigit() for char in pred)
    # This was filtering out FCFs that only contain letter references (A, B, C, etc.)
